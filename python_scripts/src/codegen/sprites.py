...
def extract_sprite(graphics_data, file_addr, width_bytes, height, palette='day'):
    ...
    """
    # Use CpcPalette to get colors (Pen 0, 1, 2, 3)
    palette_colors = CpcPalette.get_palette_for_rendering(palette)
    ...
    for y in range(height):
        for x_byte in range(width_bytes):
            ...
            for px_num, color_idx in enumerate(decoded_pixels):
                ...
                # In game sprites, logic usually treats Pen 3 (Black) as transparent background
                # OR Pen 0? Let's check the previous logic.
                # Previous logic: if color_idx == 0: Transparent.
                # Visual Palette: 0=White, 1=Cyan, 2=Yellow, 3=Black.
                # If 0 is White, it's NOT transparent.
                # If 3 is Black, it IS transparent.
                # The game sprites are drawn with masking.
                # For extraction, usually the "Background" color is the transparent one.
                # Let's assume Pen 3 (Black) is the transparent one for sprites.
...
def extract_all_sprites(asm_file):
    ...
    base_dir = 'python_scripts/resources/sprites'
    ...
    for idx, (name, file_addr, width_bytes, height) in enumerate(SPRITE_DEFINITIONS):
        ...
        for palette in ['day', 'night']:
            ...
            filename = os.path.join(base_dir, palette, f'{idx:02d}_{name}.png')
            img.save(filename)
...
def create_sprite_sheet(palette='day'):
    ...
    # Get background color (Pen 3)
    bg_rgb = CpcPalette.get_palette_for_rendering(palette)[3]
    sheet = Image.new('RGB', (sheet_width, sheet_height), bg_rgb)
    ...
    output_file = f'python_scripts/resources/abbey_sprites_sheet_{palette}.png'
