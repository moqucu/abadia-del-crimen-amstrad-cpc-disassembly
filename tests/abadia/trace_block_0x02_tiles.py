#!/usr/bin/env python3
"""
Trace which tiles Block 0x02 actually uses during execution
"""
import os
import sys
...
class TrackedCanvas(AbbeyCanvas):
    ...
    def draw_tile(self, tile_img, x_tile, y_tile):
        # Try to identify which tile this is by checking against known tiles
        super().draw_tile(tile_img, x_tile, y_tile)
        self.painted_tiles.append((x_tile, y_tile, tile_img))

print("="*80)
print("BLOCK 0x02 TILE USAGE TRACE")
print("="*80)

block_def = BLOCK_DEFINITIONS[0x02]
print(f"\nBlock 0x02 tile_data: {[f'0x{t:02X}' for t in block_def.tile_data]}")
...
# Test with different parameters to see if different tiles get used
test_cases = [
    (1, 0),
    (1, 1),
    (3, 3),
...
for param1, param2 in test_cases:
    ...
    for _, _, tile_img in canvas.painted_tiles:
        # Check against all tiles in tile_data
    ...
    print(f"Unique tiles used: {[f'0x{t:02X}' for t in sorted(tiles_used)]}")

    # Check interpreter registers to see what tiles are loaded
    print(f"\nInterpreter registers after execution:")
    for i in range(1, 13):
        if i in interpreter.regs:
            print(f"  reg[{i}] = 0x{interpreter.regs[i]:02X}")
